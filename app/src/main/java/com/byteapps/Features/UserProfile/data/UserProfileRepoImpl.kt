...
class UserProfileRepoImpl @Inject constructor(private val db:FirebaseFirestore,private val auth:FirebaseAuth,private val storage: FirebaseStorage) : UserProfileRepository{
    ...
    override suspend fun uploadUserImage(imageUri: Uri): Flow<ResultState<String>> = callbackFlow {
        ...
        storage.getReference("UserProfileImages").child(auth.currentUser?.phoneNumber.toString()).putFile(imageUri)
    ...
    override suspend fun isUserExist(userUID:String): Flow<ResultState<Boolean>> = callbackFlow {

        trySend(ResultState.Loading)

        val documentReference = db.collection("UsersProfile").document("SuwBUcD7IuUzFgPDDoBiGXL61kF3")
